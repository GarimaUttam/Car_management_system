...
func (e EngineStore) EngineById(ctx context.Context, id string) (models.Engine, error){
		...
		err = tx.QueryRowContext(ctx, "SELECT id, displacement, no_of_cylinders, car_range FROM engine WEHERE id=$1", id).Scan(
...
func (e EngineStore) EngineCreate(ctx context.Context, engineReq *models.EngineRequest) (models.Engine, error,) {
		...
		_, err = tx.ExecContext(ctx,"INSERT INTO engine (id, displacemnt, no_of_cylinders, car_range) VALUES ($1,$2,$3,$4)", engineID, engineReq.Displacement, engineReq.NoOfCylinders, engineReq.CarRange)
